...
class Users(db.Model):
    id = db.Column(
        db.Integer, Sequence("users_id_seq", start=1, increment=1), primary_key=True
    )
                ...
                "Messages": [
                    {
                        ...
                        "To": [{"Email": usuario_email, "Name": user.nombre}],
...
class Actividades(db.Model):
    id = db.Column(
        db.Integer,
        Sequence("actividades_id_seq", start=1, increment=1),
        primary_key=True,
    )
    ...
    def get_by_guia(self, pid):
        ...
        return self.query.filter_by(id_guia=pid, activo=1).order_by(
            Actividades.fecha.desc()
        )
    ...
    def act_index(self):
        return (
            self.query.filter_by(activo=1)
            .order_by(Actividades.calificacion.desc())
            .limit(6)
        )
    ...
    @classmethod
    def modifica_by_id(self, pid, data):  # ACTIVIDAD
        actividad = self.query.get(pid)
        flash("Modifica actividad", data["nombre"])
        print("hola", data)
        if data:
            # print(data)
            actividad = self.query.get(pid)
        if actividad:
            ...
            if lista_ids_usuarios:
                ...
                if data["nombre"] and actividad.nombre != data["nombre"]:
                    actividad.nombre = data["nombre"]
                else:
                    actividad.nombre = actividad.nombre
                if data["descripcion"] and actividad.descripcion != data["descripcion"]:
                    actividad.descripcion = data["descripcion"]
                else:
                    actividad.descripcion = actividad.descripcion
                if data["precio"] and actividad.precio != data["precio"]:
                    actividad.precio = data["precio"]
                else:
                    actividad.precio = actividad.precio
                if data["fecha"] and actividad.fecha != data["fecha"]:
                    actividad.fecha = data["fecha"]
                else:
                    actividad.fecha = actividad.fecha
                if data["ciudad"] and actividad.ciudad != data["ciudad"]:
                    actividad.ciudad = data["ciudad"]
                else:
                    actividad.ciudad = actividad.ciudad
            else:
    ...
    def desactiva_by_id(self, pid):
        ...
        if act and act.activo == 1:
            ...
            if fecha_act > hoy:
                ...
                if (
                    act.ids_usuarios is not None and act.ids_usuarios != ""
                ):  # si la actividad está en fecha y existe algo en el campo ids_usuarios enviar mail y cancelar reservas
...
class Reservas(db.Model):
    id = db.Column(
        db.Integer, Sequence("reservas_id_seq", start=1, increment=1), primary_key=True
    )
    num_reserva = db.Column(
    ...
    id_actividad = db.Column(
        db.Integer, ForeignKey("actividades.id"), unique=False, nullable=False
    )
                ...
                "Messages": [
                    {
                        ...
                        "To": [{"Email": guia.email, "Name": guia.nombre}],
    ...
    def res_nueva(self, datos):
        ...
        if guia:
            txt_mail = (
                "Hola, El usuario "
                + usuario.nombre
                + " ha hecho una reserva de la actividad "
                + actividad.nombre
                + " con número de reserva "
                + num_reserva_g
            )
            # print(txt_mail)
            data = {
...
class Comentarios(db.Model):
    id = db.Column(
        db.Integer,
        Sequence("comentarios_id_seq", start=1, increment=1),
        primary_key=True,
    )
    id_actividad = db.Column(
        db.Integer, ForeignKey("actividades.id"), unique=False, nullable=False
    ...
    def com_nuevo(self, id_act, id_usr, data):
        new_comn_act = Comentarios(
            id_actividad=id_act, id_usuario=id_usr, texto=data["texto"], activo=1
        )
